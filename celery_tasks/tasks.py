...
@celery_app.task(name='watchcord.celery_tasks.tasks.random_task')
def random_task(name):
    print(f"Hello {name}")
    return "Hello"+name

@celery_app.task(name='watchcord.celery_tasks.tasks.scrape_amazon_task')
...
# @celery_app.task(name='watchcord.celery_tasks.tasks.scrape_amazon_with_webhook_task')
...
def schedule_tasks_with_redbeat(urls, schedule_for, user):
    ...
    entry = RedBeatSchedulerEntry(name="scrape_amazon_with_webhook_task"+str(uuid4()), task="watchcord.celery_tasks.tasks.scrape_amazon_with_webhook_task", schedule=interval, args=[urls, user], app=celery_app)
    entry.save()
